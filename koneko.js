...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '6282141099076@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
newReply(`*「  NCBOTZ-MD ꜱᴄʀɪᴘᴛ  」*\n\n*YouTube:*\n*https://instagram.com/nanocendysstore*\n\n*Don‘t Forget to Donate Yeah* 🍜 `)
        break
...
break
            ...
            case 'attp':
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} NCBOTZ-MD`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By NCBOTZ-MD.mp3`}, { quoted : m })
        ...
        break
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By NCBOTZ-MD\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By NCBOTZ-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'carvedwood':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By NCBOTZ-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
    ...
    // Text Prome
		...
		case 'wallgravity':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${apikey}&text1=${args[0]}&text2=${args[1]}`}, caption: `Created By NCBOTZ-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
		...
		case 'thunder':
            ...
            db.data.users[m.sender].limit -= 2 // -1 limit
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${apikey}&text=${args}` }, caption: `Created By NCBOTZ-MD\n\n Type: ${command}\n\nText: ${args}`})
			break
        ...
        break
            ...
            if (!text) return newReply(`Example: ${prefix + command} NCBOTZ-MD`)
...
break
            ...
            case 'menu':
case 'help': {
    let anu =`Hello ${pushname}\nI Am ${namabot} a WhatsApp Bot that can make it easier for you to Solved Problems with the Internet quickly.
...
let e = String(err)
conn.sendMessage("6282141099076@s.whatsapp.net", { text: "👋🏻 Hello Developer, ada yang Error nih di bagian " + util.format(e), 
contextInfo:{
